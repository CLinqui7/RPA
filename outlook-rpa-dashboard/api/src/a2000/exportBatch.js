...
function isImportableOrder(order) {
  const hasHeader = !!(
    clean(order.customer_code) &&
    clean(order.store_code || order.store_raw) &&
...
function codeOrBlank(code, raw = '') {
  const c = clean(code);
  if (c) return c;
  const r = clean(raw);
  // Store numbers, dept numbers and style-like values are often safe as raw codes.
  // Customer names like "Gabe's" or "Bealls" are not safe customer codes, so keep blank unless mapped.
  return isCodeLike(r) ? r : '';
}
...
function orderToA2000HeaderRow(order) {
  ...
  row.STORE_NO = codeOrBlank(order.store_code, order.store_raw);
  ...
  row.BOOK_DATE = formatDateForA2000(order.book_date || order.order_date);
  ...
  row.TERM_NO = clean(order.terms_code) || mapTermsCode(order.terms_raw);
  ...
  row.MASTER_INVOICE = clean(order.master_invoice || 'Y');
...
function lineToA2000LineRow(order, line) {
  ...
  row.SIZE_NO = clean(line.size_code);
...
export async function createDemoA2000Batch({
  ...
  const { data: batch, error: batchError } = await supabase
    ...
    .insert({
      ...
      raw_json: {
        note: 'IMPORT-SAFE DEMO MODE: exports only importable parsed orders in the exact A2000 distro header and distro lines layouts supplied by the functional import examples. Missing A2000 mappings remain blank or raw where safe.',
        includeNeedsMapping,
